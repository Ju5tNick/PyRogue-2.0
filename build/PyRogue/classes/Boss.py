...
class Boss(pygame.sprite.Sprite):
    ...
    def move(self, hero, hero_group, game_params):
        ...
        if self.angry and self.can_move:
            ...
            if abs(hero.get_coords()[0] - self.rect.x) <= 100 and abs(
                    hero.get_coords()[
                        1] - self.rect.y) <= 100 and not self.die_flag and self.angry and not self.is_attack and not self.spawn:
    ...
    def attack(self, game_params):
        ...
        for i in _list:
            for j in _list:
                self.game_params["crack_group"].add(
                    Crack([self.rect.center[0] + i, self.rect.center[1] + j], 10, game_params))
    ...
    def get_damage(self, damage):
        ...
        if self.current_health <= 0 and not self.die_flag:
            ...
            Sound.play(SOUNDS["GAME"]["boss-win"])
